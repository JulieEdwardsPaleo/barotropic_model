...
#            levels = np.linspace(-30., 30., 16)
        ...
        if field in ['psi', 'strf']:
            label = 'Streamfunction anomaly [m2/s]'
            cmap = plt.get_cmap('RdBu_r')

            levsneg = np.linspace(-2.e7, -.2e7, 10)
            levspos = np.linspace(.2e7, 2.e7, 10)
            levels = np.concatenate((levsneg, levspos))

            if diff is True:
                label = 'Streamfunction anomaly [m]'
                cmap = plt.get_cmap('RdBu_r')
                levels = np.linspace(-1.e7, 1.e7, 21)
        ...
        if field in ['topo', 'srfgeo']:
            ...
            if diff is True:
                ...
                cmap = plt.get_cmap('RdBu_r')
                levels = np.linspace(-1000., 10000., 11)
...
###
    ...
    def plot_movie(self, lats, lons0, var, topo,
                   field='psi', diff=True, add_cyclic=True): # Adds in coastlines and fixes need for X and Y compared to old version

        cmap, levels, label = self.settings(field, diff=diff)

        if add_cyclic is True:
            var, lons = add_cyclic_point(var, coord=lons0)
        else:
            lons = lons0
        ...
        if add_cyclic is True:
            topo, _ = add_cyclic_point(topo, coord=lons0)

        cmapTopo, levelsTopo, labelTopo = self.settings('topo')
        cont = self.ax.contour(lons, lats, topo, levelsTopo,
                               colors='k', transform=self.trans)

        if self.add_colorbar is True:
            cbar = self.fig.colorbar(cf, shrink=0.9, extend='both',
                                     orientation='horizontal')
